...
export class Transform {
  ...
  async list(): Promise<any> {
    const req = new HookSniffRequest(HttpMethod.GET, "/v1/transforms");
    return req.send(this.ctx, (j) => j);
  }

  /** Create a transform. */
  async create(body: Record<string, any>): Promise<any> {
    const req = new HookSniffRequest(HttpMethod.POST, "/v1/transforms");
    req.setBody(body);
    return req.send(this.ctx, (j) => j);
  }
  ...
  async update(id: string, body: Record<string, any>): Promise<any> {
    const req = new HookSniffRequest(HttpMethod.PUT, `/v1/transforms/${id}`);
    req.setBody(body);
    return req.send(this.ctx, (j) => j);
  }
  ...
  async delete(id: string): Promise<void> {
    const req = new HookSniffRequest(HttpMethod.DELETE, `/v1/transforms/${id}`);
    return req.sendNoResponseBody(this.ctx);
  }

  /** Test a transform. */
  async test(body: Record<string, any>): Promise<any> {
    const req = new HookSniffRequest(HttpMethod.POST, "/v1/transforms/test");
    req.setBody(body);
    return req.send(this.ctx, (j) => j);
  }
}
