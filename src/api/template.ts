...
export class Template {
  ...
  async list(): Promise<any> {
    const req = new HookSniffRequest(HttpMethod.GET, "/v1/templates");
    return req.send(this.ctx, (j) => j);
  }

  /** Get a template. */
  async get(id: string): Promise<any> {
    const req = new HookSniffRequest(HttpMethod.GET, `/v1/templates/${id}`);
    return req.send(this.ctx, (j) => j);
  }

  /** Apply a template. */
  async apply(id: string, body?: Record<string, any>): Promise<any> {
    const req = new HookSniffRequest(HttpMethod.POST, `/v1/templates/${id}/apply`);
