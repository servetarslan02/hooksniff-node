...
export class Application {
  ...
  async list(params?: Record<string, string>): Promise<any> {
    const req = new HookSniffRequest(HttpMethod.GET, "/api/v1/applications");
  ...
  async create(body: Record<string, any>): Promise<any> {
    const req = new HookSniffRequest(HttpMethod.POST, "/api/v1/applications");
    req.setBody(body);
    return req.send(this.ctx, (j) => j);
  }
  ...
  async get(id: string): Promise<any> {
    const req = new HookSniffRequest(HttpMethod.GET, `/api/v1/applications/${id}`);
    return req.send(this.ctx, (j) => j);
  }

  /** Update an application. */
  async update(id: string, body: Record<string, any>): Promise<any> {
    const req = new HookSniffRequest(HttpMethod.PUT, `/api/v1/applications/${id}`);
    req.setBody(body);
    return req.send(this.ctx, (j) => j);
  }
  ...
  async delete(id: string): Promise<void> {
    const req = new HookSniffRequest(HttpMethod.DELETE, `/api/v1/applications/${id}`);
