...
export class Device {
  ...
  async list(): Promise<any> {
    const req = new HookSniffRequest(HttpMethod.GET, "/v1/devices");
    return req.send(this.ctx, (j) => j);
  }

  /** Register a device. */
  async register(body: Record<string, any>): Promise<any> {
    const req = new HookSniffRequest(HttpMethod.POST, "/v1/devices");
    req.setBody(body);
    return req.send(this.ctx, (j) => j);
  }
  ...
  async delete(id: string): Promise<void> {
    const req = new HookSniffRequest(HttpMethod.DELETE, `/v1/devices/${id}`);
