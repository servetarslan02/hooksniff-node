...
export class RateLimit {
  ...
  async list(): Promise<any> {
    const req = new HookSniffRequest(HttpMethod.GET, "/api/v1/rate-limits");
    return req.send(this.ctx, (j) => j);
  }

  /** Get rate limit for endpoint. */
  async get(endpointId: string): Promise<any> {
    const req = new HookSniffRequest(HttpMethod.GET, `/api/v1/rate-limits/${endpointId}`);
    return req.send(this.ctx, (j) => j);
  }

  /** Set rate limit. */
  async set(endpointId: string, body: Record<string, any>): Promise<any> {
    const req = new HookSniffRequest(HttpMethod.POST, `/api/v1/rate-limits/${endpointId}`);
    req.setBody(body);
    return req.send(this.ctx, (j) => j);
  }
  ...
  async delete(endpointId: string): Promise<void> {
    const req = new HookSniffRequest(HttpMethod.DELETE, `/api/v1/rate-limits/${endpointId}`);
