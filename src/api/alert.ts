...
export class Alert {
  ...
  async list(): Promise<any> {
    const req = new HookSniffRequest(HttpMethod.GET, "/v1/alerts");
    return req.send(this.ctx, (j) => j);
  }

  /** Create an alert. */
  async create(body: Record<string, any>): Promise<any> {
    const req = new HookSniffRequest(HttpMethod.POST, "/v1/alerts");
    req.setBody(body);
    return req.send(this.ctx, (j) => j);
  }
  ...
  async get(id: string): Promise<any> {
    const req = new HookSniffRequest(HttpMethod.GET, `/v1/alerts/${id}`);
    return req.send(this.ctx, (j) => j);
  }

  /** Update an alert. */
  async update(id: string, body: Record<string, any>): Promise<any> {
    const req = new HookSniffRequest(HttpMethod.PUT, `/v1/alerts/${id}`);
    req.setBody(body);
    return req.send(this.ctx, (j) => j);
  }
  ...
  async delete(id: string): Promise<void> {
    const req = new HookSniffRequest(HttpMethod.DELETE, `/v1/alerts/${id}`);
    return req.sendNoResponseBody(this.ctx);
  }

  /** Test an alert. */
  async test(id: string): Promise<any> {
    const req = new HookSniffRequest(HttpMethod.POST, `/v1/alerts/${id}/test`);
    return req.send(this.ctx, (j) => j);
  }
}
