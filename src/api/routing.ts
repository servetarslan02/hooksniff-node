...
export class Routing {
  ...
  async get(endpointId: string): Promise<any> {
    const req = new HookSniffRequest(HttpMethod.GET, `/v1/routing/${endpointId}/routing`);
    return req.send(this.ctx, (j) => j);
  }

  /** Update routing. */
  async update(endpointId: string, body: Record<string, any>): Promise<any> {
    const req = new HookSniffRequest(HttpMethod.PUT, `/v1/routing/${endpointId}/routing`);
    req.setBody(body);
    return req.send(this.ctx, (j) => j);
  }
  ...
  async getHealth(endpointId: string): Promise<any> {
    const req = new HookSniffRequest(HttpMethod.GET, `/v1/routing/${endpointId}/health`);
    return req.send(this.ctx, (j) => j);
  }
}
